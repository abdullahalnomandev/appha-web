import { ITeamContact } from "@/types/main";
...
const ContactsTab = ({data}: { data: ITeamContact[] }) => (
  <div className="space-y-6">
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
      {data.map((c) => (
        <div key={c.name} className="rounded-lg border border-gray-200 p-5 shadow-sm">
          <h4 className="text-gray-900 font-semibold">{c.name}</h4>
          <p className="text-xs text-amber-500 mb-3">{c.title}</p>
    ...
    <div className=" rounded-lg border border-amber-200 p-6 text-center shadow-sm">
