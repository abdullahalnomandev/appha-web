...
function VerifyOtpProfessional({ email }: { email: string }) {
  ...
  const verifyOTP = async () => {
    const code = otp.join("");
    if (code.length !== 4) return message.error("Please enter 4-digit OTP");

    setLoading(true);
    try {
      await apiFetch("/auth/verify-reset-otp", {
        method: "POST",
        body: JSON.stringify({ email, oneTimeCode: code }),
      });
      message.success("OTP verified! Proceed to change password.");
